    ...
    <div class="row space-top">
        <div class="col-md-4">
            ${errorMsg ? html`<div class="form-group">
                <p style=${styleMap({ color: 'red' })}> ${errorMsg} </p>
            </div>` : ''}
...
export async function registerPage(ctx) {
    ...
    async function onSubmit(event) {
        ...
        if (email == '' || password == '' || repass == '') {
            return ctx.render(registerTemplate(onSubmit, 'All fields are rquired!', email == '', password == '', repass == ''));
        }
        if (password != repass) {
            return ctx.render(registerTemplate(onSubmit, 'Paswords don\'t match', false, true, true));
        }
